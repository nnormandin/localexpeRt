...
The first step in using the local expert method to solve a regression problem is to encode the continuous target variable vector as a binary matrix. This will be accomplished using indicator functions with criteria values placed throughout the expected range of the target variable. The two ways of determing the location of these 'break points' within the expected response range are:

  1. Equally Probable (EP) Intervals:  
  2. Equally Wide (EW) Intervals: 
