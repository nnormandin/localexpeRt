...
It is clear that the the relationship is imperfect (which we expected), but the p-value is undeniable evidence of a relationship. We can visualize that relationship by plotting these vectors against one another.
...
Finally, if the stacked generalization phase of this methodology is worthwhile, we should expect to see a decrease in predictive error.

```{r}
# calculate distribution's mean absolute deviation
distribution.mad <- mean(abs(y - LE.fits$mean))
distribution.mad

# calculate stacked model mean absolute deviation
stacked.mad <- mean(abs.error)
stacked.mad
```
