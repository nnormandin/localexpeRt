...
Function         | Usage
---------------- | --------------------------------------------------------------------
BinCols          | Turns continuous target vector into binary matrix
...
Note that the generation of the local expert model list is the most computationally intensive portion of this methodology, and can take several hours depending (even on modern hardware). The length of time is dependent on the:
...
We have now completed all of our steps in building our ensemble, and can now start to use it in practice. Let's pick an instance from the test set of the data we were using at random.
...
# predict y.hat
y.hat <- PredictNew(x = x.new, LE.model.list = model.list, stack.model = L1.model,
