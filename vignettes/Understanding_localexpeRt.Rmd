...
We can look at the histogram of the target variable again, with verticle lines at each of the break points. Each line represents a different local expert function, which will classify instances based on whether their associated target variable value is to the left or right of the associated line.
...
For this example, we will use gradient boosting trees (`gbm`) as the method and leave all other parameters as their default values. In order to quickly examine the 
...
cases when you would not use the short way- custom meta features (included from original model, etc..), you want to build your own plots
